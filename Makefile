...
AWK = gawk
shared_objects_doc = src/doc.lo src/scanner.lo src/parser.lo
PHP_PECL_EXTENSION = doc
PHP_MODULES = $(phplibdir)/doc.la
PHP_ZEND_EX =
...
./doc.la: $(shared_objects_doc) $(DOC_SHARED_DEPENDENCIES)
	$(LIBTOOL) --mode=link $(CC) $(COMMON_FLAGS) $(CFLAGS_CLEAN) $(EXTRA_CFLAGS) $(LDFLAGS)  -o $@ -export-dynamic -avoid-version -prefer-pic -module -rpath $(phplibdir) $(EXTRA_LDFLAGS) $(shared_objects_doc) $(DOC_SHARED_LIBADD)
